...
#Looking at the available variables, I decide to look at both educationn and household variables

contextdata <- contextdata %>% 
  select(median_hh_inc, clf_unemploy_pct, lesshs_pct, lesscollege_pct, lesshs_whites_pct, lesscollege_whites_pct, rural_pct) %>% 
  ...
  summarise(
    ...
    clf_unemploy_pct = sum(clf_unemploy_pct)/100,
    lesshs_pct = sum(lesshs_pct)/100,
    lesscollege_pct = sum(lesscollege_pct)/100,
    lesshs_whites_pct = sum(lesshs_whites_pct)/100,
    lesscollege_whites_pct = sum(lesscollege_whites_pct)/100,
    rural_pct = sum(rural_pct)/100)
